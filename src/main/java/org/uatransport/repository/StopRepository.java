...
public interface StopRepository extends CrudRepository<Stop, Integer> {
    ...
    @Query("SELECT s FROM Transit t JOIN t.stops s WHERE t.id = :id "
            + "AND s.street IS NOT NULL AND s.direction ='FORWARD'ORDER BY INDEX(s)")
    List<Stop> findByTransitId(@Param("id") Integer id);

    @Query("SELECT s FROM Transit t JOIN t.stops s WHERE t.id = :id "
            + "AND s.street IS NOT NULL AND s.direction ='FORWARD' AND s.street = :street")
    Stop findByTransitIdAndStopName(@Param("id") Integer transitId, @Param("street") String street);

    @Query("SELECT INDEX(s) FROM Transit t JOIN t.stops s WHERE t.id = :id "
            + "AND s.street IS NOT NULL AND s.direction ='FORWARD' AND s.street = :street")
    ...
    @Query("SELECT s FROM Transit t JOIN t.stops s WHERE t.id = :id "
            + "AND s.street IS NOT NULL AND s.direction ='BACKWARD' ORDER BY INDEX(s)")
