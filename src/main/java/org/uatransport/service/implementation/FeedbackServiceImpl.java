...
public class FeedbackServiceImpl implements FeedbackService {
            ...
            .orElseThrow(
                () ->
                    new ResourceNotFoundException(
                        String.format("Feedback with id '%s' not found", id)));
    }
    ...
    public <T> List<T> getByTransitAndFeedbackCriteria(Integer transitId, FeedbackCriteria.TypedFeedbackType<T> type) {
        return feedbackRepository.findByTransitIdAndFeedbackCriteriaType(transitId, type.get())
                .stream()
                .map(feedback -> type.convert())
                .collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<Feedback> getByTransitAndFeedbackCriteriaAndUserId(
        Integer transitId, FeedbackCriteria.FeedbackType feedbackType, Integer userId) {
        return feedbackRepository.findByTransitIdAndFeedbackCriteriaTypeAndUserId(
            transitId, feedbackType, userId);
    }
    ...
    public Double getAverageRateByTransitId(Integer transitId) {
        List<CapacityFeedback> feedbackList = getByTransitAndFeedbackCriteria(transitId, FeedbackCriteria.TypedFeedbackType.CAPACITY);
    ...
    public Double getAverageRateByTransitAndUser(Integer transitId, Integer userId) {
        List<Feedback> feedbackList =
            getByTransitAndFeedbackCriteriaAndUserId(
                transitId, FeedbackCriteria.FeedbackType.RATING, userId);
        return getAverageRate(feedbackList);
    }
    ...
    private Double getAverageCapacityByTransitAndHour(Integer transitId, Integer feedbackHour) {
        return convertCapacityFeedBacks(transitId)
                .stream()
                .filter(CapacityFeedback::isCapacityHoursFeedback) // TODO: this should be different feedback types
                .filter(capacityFeedback -> capacityFeedback.containsHour(feedbackHour))
    ...
    private List<CapacityFeedback> convertCapacityFeedBacks(Integer transitId) {
        return getByTransitAndFeedbackCriteria(transitId, FeedbackCriteria.FeedbackType.CAPACITY)
            ...
            .map(
                feedback ->
                    (CapacityFeedback) FeedbackCriteria.FeedbackType.CAPACITY.convertFeedback(feedback))
            .collect(Collectors.toList());
    }
    ...
    public Map<Stop, Double> getStopCapacityMap(Integer transitId, Stop... stops) {
        List<Stop> stopList =
            stops.length > 0 ? Arrays.asList(stops) : stopService.getByTransitId(transitId);
        Map<Stop, Double> capacityMap =
            new TreeMap<>(
                Comparator.comparingInt(
                    stop -> stopService.getIndexByTransitIdAndStopName(transitId, stop.getStreet())));
    ...
    private boolean existInStopIndexesRange(
        Integer transitId, Stop stop, String fromStop, String toStop) {
    ...
    private Double getCapacityByTransitAndStops(Integer transitId, Stop stop) {
        Predicate<CapacityFeedback> existInRange =
            capacityFeedback ->
                existInStopIndexesRange(
                    transitId, stop, capacityFeedback.getFrom(), capacityFeedback.getTo());
    ...
    private List<AccepterFeedback> convertAccepterFeedBacks(Integer transitId) {
        return getByTransitAndFeedbackCriteria(transitId, FeedbackCriteria.FeedbackType.ACCEPTER)
            ...
            .map(
                feedback ->
                    (AccepterFeedback) FeedbackCriteria.FeedbackType.ACCEPTER.convertFeedback(feedback))
            .collect(Collectors.toList());
    }
